...
var CHESS = 'SOD-Chess chess';
var WEATHER = 'SOD-Chess weather';
...
SOD-Chess.on('message', function(message) {
        var msg = strip(message.content);
        if (message.author.id === SOD-Chess.user.id) {
            return;
        }
        var pp = '165886688625819649';
        var umsg = msg.toLowerCase();
        if(message.isMentioned(SOD-Chess.user)) {
            if(msg.indexOf('help') !== -1) {
                message.reply('Valid commands are: `woof`, `bark`, `sleep tight pupper`, `SOD-Chess chess [algebraic chess notation]`, `SOD-Chess chess resign`, `SOD-Chess why`');
        ...
        if (umsg === 'sudo woof') {
        ...
        } else if (umsg == 'SOD-Chess why') {
            message.reply(why.why());
        } else if(umsg == 'pupper why') {
            message.reply(why.why());
            if (Math.random() < 0.3) {
                setTimeout(function() {
                        message.reply("also, it's SOD-Chess, not pupper");
                        }, 100);
            }
            ...
            if(thinking[id] === true) {
                SOD-Chess.reply(chessmsg[id], "I'm still thinking...");
                return;
            }
...
SOD-Chess.login(/* INSERT YOUR TOKEN HERE */);
console.log(SOD-Chess)